...
4. Evaluate with `eval_protocol.py` using the Friday primary condition: static/dynamic friction `0.21`, 64 environments, 300 steps, seed 7, warmup 1, all terminations disabled, and the explicit body-length and gait-period fields. Preserve the generated JSON sidecar and reward-term breakdown. The manifest's `0.215` is earlier launch metadata, not the final primary evaluation condition.
