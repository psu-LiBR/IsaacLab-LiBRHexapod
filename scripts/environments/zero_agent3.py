...
parser.add_argument("--task", type=str, default="Isaac-Velocity-Sand-Improved-Hexapod-Play-v0", help="Name of the task.")
# append AppLauncher cli args
AppLauncher.add_app_launcher_args(parser)
# parse the arguments
args_cli = parser.parse_args()

# 在 headless 录制 rgb_array 时必须启用相机渲染，否则 RecordVideo 会在 render() 时报错。
...
def main():
    ...
    task_name = args_cli.task or "Isaac-Velocity-Sand-Improved-Hexapod-Play-v0"
    ...
    video_output_dir = f"/home/ubuntu/IsaacLab-LiBRHexapod/my_videos/improved_rft/{run_stamp}"
    ...
    env = gym.wrappers.RecordVideo(
        ...
        video_length=300,
        name_prefix=f"hexapod_sand_improved_rft_demo_{run_stamp}",
    )

    env.reset()

    sim_steps = 0
    print("[INFO]: 正在测试并录制改进版沙地环境...")
    ...
    initial_pos = None
    final_pos = None
    HEXAPOD_BODY_LENGTH = 0.35  # 六足机器人的体长（米，基于OBJ文件的精确测量）
    ...
    while simulation_app.is_running():
        with torch.inference_mode():
            ...
            sim_steps += 1
            if sim_steps >= 300:
                print(f"[INFO]: 测试成功完成！视频目录: {video_output_dir}")
                ...
                if initial_pos is not None and final_pos is not None:
                    ...
                    print("\n" + "="*60)
                    print("改进版沙地环境（zero_agent3）移动统计")
                    print("="*60)
