...
def test_csv_run_logger_writes_header_and_rows(tmp_path):
    ...
    with CsvRunLogger(str(path), obs_dim=obs_dim, action_dim=action_dim) as logger:
        logger.log(
            ...
            target_sim=np.zeros(NUM_JOINTS),
            target_ticks=np.zeros(NUM_JOINTS),
            measured_pos_sim=np.zeros(NUM_JOINTS),
            measured_vel_sim=np.zeros(NUM_JOINTS),
            loop_time_ms=5.0,
        )
        logger.log(
            step=1,
            ...
            target_sim=np.ones(NUM_JOINTS),
            target_ticks=np.full(NUM_JOINTS, 2048),
            measured_pos_sim=np.ones(NUM_JOINTS),
            measured_vel_sim=np.ones(NUM_JOINTS),
            loop_time_ms=6.0,
        )
